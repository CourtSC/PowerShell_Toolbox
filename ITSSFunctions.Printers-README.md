...
```powershell
Get-PrintersWithErrors -Servers 'PRINT01','PRINT02' -Remove
```
...
```powershell
Get-PrintersWithErrors -Servers 'PRINT01' -Nuke
```
...
```powershell
'PC01','PC02' | Install-PrinterDriver -Credential (Get-Credential) -Verbose
```
...
```powershell
$s = New-PSSession -ComputerName 'PC01','PC02'
...
This function fixes misconfigured TCP/IP ports for a given printer on a target print server. Because there is no PowerShell cmdlet to modify an existing port in place, it:
...
```powershell
Set-PrinterPort -Name 'HP-LJ-05' -Server 'PRINT01' -HostAddress '10.12.34.56' -Verbose
```

Fixes the port configuration for a single printer.

#### Example 2

```powershell
Set-PrinterPort -Name 'HP-LJ-05' -Server 'PRINT01' -HostAddress '10.12.34.56' -Force
```

Runs non-interactively, while still supporting `-WhatIf` / `-Confirm`.

#### Example 3

```powershell
[pscustomobject]@{ Name='HP-A1'; Server='PRINT01'; HostAddress='prn-a1.contoso.com' } | Set-PrinterPort -Force
```

Pipes objects with the required properties.
...
Lists per-user printer connections from one or more computers or `PSSession` objects.

### DESCRIPTION

Enumerates registry keys under `HKEY_USERS\<SID>\Printers\Connections` on the target system(s) to report installed (connected) printers for each user profile. Works against:

- One or more computer names via PowerShell remoting, or
- One or more existing `PSSession` objects.

For each connection, returns a structured object with `ComputerName`, `SID`, optionally resolved `User` name, and the connection name (`Printer`).

### PARAMETERS

#### -ComputerName

One or more remote computer names to query via PowerShell remoting. Accepts pipeline input. Requires WinRM connectivity and appropriate permissions.

#### -Session

One or more existing `PSSession` objects to use for the query. Accepts pipeline input.

#### -ResolveUser

When specified, attempts to resolve each SID to an `NTAccount` (`DOMAIN\User`). If resolution fails, the SID is returned as-is.
...
Specifies a user account that has permission to run the command on the remote computer(s). Only applicable with `-ComputerName`. When defined with the `[Credential()]` attribute and supplied without a value, you will be prompted for credentials.
...
```powershell
Get-InstalledPrinters -ComputerName 'PC01'
```

#### Example 2: Query multiple computers with resolved users

```powershell
'PC01','PC02' | Get-InstalledPrinters -ResolveUser
```
...
```powershell
$s = New-PSSession -ComputerName 'PC01','PC02'
Get-InstalledPrinters -Session $s
```

#### Example 4: Capture and export

```powershell
Get-InstalledPrinters -ComputerName 'PC01','PC02' -ResolveUser | Export-Csv printers.csv -NoTypeInformation
```

#### Example 5: Use alternate credentials

```powershell
Get-InstalledPrinters -ComputerName 'PC01','PC02' -Credential (Get-Credential)
```

### NOTES

- Reads HKU (user hives) on target machines: `HKEY_USERS\<SID>\Printers\Connections`.
- Per-machine printers under HKLM are not included.
...
By default, `Prefix` is `ORL-PRN-` and `DefaultMarker` is `DEF`. The function returns any groups that exist and ignores missing ones (reported via `-Verbose`). You can request additional AD properties with `-Properties`; the function will also include `Description` and `info` unless you request `*` (all properties).
...
Additional AD group properties to retrieve. If `*` is specified, all properties are returned. Otherwise the function ensures `Description` and `info` are included.
...
```powershell
Get-ADPrinterGroups -PrinterName 'IBJ9' -Verbose
'IBJ9','HP-5100' | Get-ADPrinterGroups -Properties ManagedBy
Get-ADPrinterGroups -PrinterName 'X123' -Prefix 'MCO-PRN-' -DefaultMarker 'DEFAULT'
Get-ADPrinterGroups -PrinterName 'HP-Color' -Server 'dc01.contoso.com' -SearchBase 'OU=Print,DC=contoso,DC=com'
```
...
The first example retrieves printers created in the last 6 months from the default domain and exports the results to Excel. The second retrieves printers created after January 1, 2024, from the `multihosp.net` domain and exports the results to Excel.
